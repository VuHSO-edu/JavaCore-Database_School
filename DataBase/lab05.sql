...
SELECT concat(HONV,TENLOT,TENNV) AS TENDAYDU
FROM NHANVIEN
WHERE HONV LIKE 'N%';
...
SELECT concat(HONV,' ',TENLOT,' ',TENNV), year(NGSINH)
FROM NHANVIEN;

-- question09

SELECT concat(HONV,' ',TENLOT,' ',TENNV),year(curdate()) -  year(NGSINH) AS TUOI
FROM NHANVIEN;
